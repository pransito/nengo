...
with model:
    ...
    stim_1 = nengo.Node(piecewise({0: 1}))
    #stim_1 = nengo.Node([0])
    #stim_2 = nengo.Node([0])
    ...
    output = nengo.Ensemble(n_neurons=1000, dimensions=1,radius=6)
    ...
    spikes     = nengo.Probe(output.neurons) # The raw spikes from the neuron
    voltage    = nengo.Probe(output.neurons, 'voltage')  # Subthreshold soma voltage of the neuron
    filtered   = nengo.Probe(output, synapse=0.01)       # Spikes filtered by a 10ms post-synaptic filter
...
plt.title('Postsynaptic Potentials at output')
