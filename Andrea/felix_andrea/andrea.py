...
_gamma    = 6    # 5HT connectivity to amy, reaction to aversive stimuli (gamma > beta)? # draft paper says sigma = 0.75
...
with model:
    ...
    with model_ANDREA:
        ...
        # DA
        # DA synapse according to http://compneuro.uwaterloo.ca/research/constants-constraints/neurotransmitter-time-constants-pscs.html
        nengo.Connection(da_da, vs, synapse=syn)
        nengo.Connection(da_da, amyg, transform=_beta, synapse=0.007) 
    ...
    stim_pb     = nengo.Probe(stim_ofc) # The raw spikes from the neuron OR VALUE?!?
    ...
    ofc_out_pb  = nengo.Probe(ofc_out)
    #con_pb     = nengo.Probe(control, synapse=0.01)
    #con_PE_pb  = nengo.Probe(ctrl_PE_int, synapse=0.01)
    
    #amyg_out_probe = nengo.Probe(amyg, synapse = 0.01)
    amyg_spikes = nengo.Probe(amyg.neurons, synapse = 0.01)
    amyg_psps = nengo.Probe(amyg.neurons,'voltage')
    ...
    # sio.savemat('test_output.mat', sim.data) 


    
    
    hansi = type(sim.data[stim_pb])
    
    amyg_psps_array = sim.data[amyg_psps]
    amyg_psps_sum   = np.sum(amyg_psps_array,axis=1)
    sim.data[stim_pb].tofile('C:\\Users\\genaucka\\Google Drive\\Library\\nengo\\Andrea\\felix_andrea\\ofc_stim.txt', sep="\r\n", format="%s")
    np.savetxt('C:\\Users\\genaucka\\Google Drive\\Library\\nengo\\Andrea\\felix_andrea\\amy_psps_sum.txt',amyg_psps_sum,delimiter='\t')
    #sim.data[amyg_psps].tofile('C:\\Users\\genaucka\\Google Drive\\Library\\nengo\\Andrea\\felix_andrea\\amy_psps.txt', sep="\r\n", format="%s")
    #sim.data[stim_pb].tofile('T:\\Dokumente\\Gambling\\nengo\\testfile.txt', sep="\r\n", format="%s")
    #sim.data[amyg_spikes].tofile('T:\\Dokumente\\Gambling\\nengo\\testfile_2.txt', sep=" ", format="%s")
    ...
    plt.title(hansi) #'Postsynaptic Potentials at OFC integrator?')
    ...
    # plt.title(hansi) #'Postsynaptic Potentials at OFC integrator?')
    # plt.subplot(3,1,2)
    # rasterplot(sim.trange(), sim.data[amyg_spikes])
    # plt.xlim(0,sim_t)
    # plt.xlabel('time (s)')
    # plt.ylabel('amygdala spikes')
    # plt.title('Postsynaptic Potentials at OFC ANDREA output')
    
    plt.subplot(3,1,2)
    scale = 2
    for i in range(30): #range(amyg.n_neurons):
        plt.plot(sim.trange(), sim.data[amyg_psps][:,i] - i*scale)
    #plt.xlim(0, 1)
    #plt.ylim(scale * (-amyg.n_neurons+1), scale)
    plt.ylabel("PSPS at Amygdala")
    #plt.yticks(np.arange(scale/1.8, (-amyg.n_neurons + 1) * scale, -scale), np.arange(amyg.n_neurons));
    
    # in the third we will print the sum of the voltages (sum of 500 time serieses)
    plt.subplot(3,1,3)
    plt.plot(sim.trange(), amyg_psps_sum)
    plt.xlim(0,sim_t)
    plt.xlabel('time (s)')
    plt.ylabel('sum of PSPS of Amygdala')
    plt.title('Amygdala')
    
    plt.show()

    
# print sim.data[filt_ofc_i]
